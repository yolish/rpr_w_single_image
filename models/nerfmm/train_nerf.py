# Code copied from: https://github.com/ActiveVisionLab/nerfmm with minor adaptations
import sys
import os
import argparse
...
from models.nerfmm.dataloader.any_folder_apr import DataLoaderAnyFolder
from models.nerfmm.utils.training_utils import set_randomness, mse2psnr, save_checkpoint
from models.nerfmm.utils.pos_enc import encode_position
from models.nerfmm.utils.volume_op import volume_sampling_ndc, volume_rendering
...
def parse_args():
    ...
    parser.add_argument('--scene_name', type=str, default='/mnt/data/Cambridge/')
    parser.add_argument('--img_names_file', type=str, default='../../datasets/CambridgeLandmarks/abs_cambridge_pose_sorted.csv_ShopFacade_train.csv')
    parser.add_argument('--results_name', type=str, default='shop_facade')
...
def model_render_image(c2w, rays_cam, t_vals, near, far, H, W, fxfy, model, perturb_t, sigma_noise_std,
    ...
    # YS: Bottleneck - goes row by row and computes the rgb denisty
...
def main(args):
    ...
    '''Create Folders'''
    exp_root_dir = Path(os.path.join('./nerf_results', args.results_name))
    ...
    shutil.copy('train_nerf.py', experiment_dir)
    ...
    '''Data Loading'''
    scene_train = DataLoaderAnyFolder(base_dir=args.scene_name,
                                      img_names_file=args.img_names_file,
